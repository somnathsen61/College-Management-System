...
{
    $search = $_POST['search'];
    $sql = "SELECT * FROM `multiusercontrol`.`sixthsemcst` WHERE CONCAT(`name`,`enrollment`) LIKE '%".$search."%'";
...
<body>
    <h3>Sixth Semester CST Student Results</h3>
    <form action="table_6thSemResCST.php" method="post">
        <div class="search">
        ...
        <table>
            ...
            <?php
                ...
                while($row = mysqli_fetch_assoc($result)){
                    echo "<tr><td>". $row['name']. "</td><td>". $row['enrollment']."</td><td>". $row['os']. "</td><td>". $row['cn']. "</td><td>". $row['swe'].  "</td><td>". $row['crypto']. "</td><td>". $row['nia']. "</td><td>". $row['osLab']. "</td><td>". $row['cnLab']. "</td><td>". $row['sweLab']."</td><td><a class='button' href='\project\admin\pdf\marksheet.php?enrollment=$row[enrollment]'>Print"."</td><td><a class='button' href='\project\admin\pdf\certificate.php?enrollment=$row[enrollment]'>Send</td></tr>";
                }
                echo "</table>";
